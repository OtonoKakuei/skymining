...
public class HttpURLConnectionExt {
	...
	// we need to know the Pair <Integer, Object> where Integer value is tableId, Object is an objId - the key from that table.
	// f.e., we have table T, let's table T have TableId = 1;
	// In the internal database table T have the key column "id". All ids from table T are unique.
	// suchwise we need to know only ids to identify the object from table T
	// Thus, pair <tableId, objId (= id in our case)> show us which tuples the query "touched"
	public List<Pair<Table, Object>> sendGetResultFromQuery(RowInfo ri, HashMap<String, Table> tables) throws Exception {
		        ...
		        while ((inputLine = in.readLine()) != null) {
		        	if (iLineCount < 2)
		        	{
		        		...
		        		if ((iLineCount == 1) && isOk)
		        		{
		        			...
		        			// now we have the names of columns in the result set.
		        			// we need to find the match with key columns for tables in the from clause
