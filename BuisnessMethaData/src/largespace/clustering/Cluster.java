...
public class Cluster {
	...
	public void expand(Query point, List<Query> neighbors, List<Query> data, boolean[] visited,
			boolean[] isClusterMember, Options opt) {
		points.add(point);
		...
		while (it.hasNext()) {
			...
			if (current != point && !visited[curID] && !isClusterMember[curID] // Uncomment to avoid comparing to already clustered queries
				...
				visited[curID] = true;
				// Added Visited and Is cluster to region to avoid Comparisons
				List<Query> curNeighbors = current.region(data, visited, isClusterMember, opt);
