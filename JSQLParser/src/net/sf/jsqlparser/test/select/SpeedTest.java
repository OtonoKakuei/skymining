...
public class SpeedTest extends TestCase {
	...
	public void testSpeed() throws Exception {
		...
		ArrayList<Statement> parsedSelects = new ArrayList<Statement>(NUM_REPS * statementsList.size());
		long time = System.currentTimeMillis();

		// measure the time to parse NUM_REPS times all statements in the 2 files
		for (int i = 0; i < NUM_REPS; i++) {
			try {
				for (Iterator<String> iter = statementsList.iterator(); iter.hasNext();) {
		...
		for (Iterator<Statement> iter = parsedSelects.iterator(); iter.hasNext();) {
